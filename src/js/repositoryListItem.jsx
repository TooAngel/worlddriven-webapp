...
export class RepositoryListItem extends React.Component {
  ...
  async getPullRequests() {
    const response = await fetch(
      `/api/v1/repositories/${this.props.repository.fullName}/pulls`
    );
   ...
   **/
  handleChange(event) {
    const updateRepository = new Request(
      `/api/v1/repositories/${event.target.name}/`,
      {
        method: 'PUT',
  ...
  render() {
    ...
    for (const pullRequest of this.state.pullRequests) {
      pullRequests.push(
        <li
          ...
          onClick={() => this.selectPullRequest(pullRequest.number)}
        >
          {pullRequest.title}
        </li>
      );
    ...
    return (
      <div key={this.props.repository.fullName} className={styles.repository}>
        <div
          className={styles.repositoryName}
          onClick={() =>
            this.props.setRepository(this.props.repository.fullName)
          }
        >
          {this.props.repository.fullName}
        </div>
        <div>
          <label className={styles.switch}>
            <input
              type="checkbox"
              defaultChecked={this.props.repository.configured}
              name={this.props.repository.fullName}
